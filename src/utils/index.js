export const getRelativeSize = (size) => {
  ...
  export function getTextColorForBackground(hexColor) {
    // Remove the hash (#) if it exists in the hex color
    ...
    if (luminance > 0.7) {
      return '#000'; // Use black text for light backgrounds
    } else {
...
export const calculatePrice = (options) => {
  ...
  return {
    extraWorkspacesPrice,
    extraStoragePrice: options.subscription === 'monthly' ? monthlyPrice : yearlyPrice,
    basePrice: options.subscription === 'monthly' ? baseMonthlyPrice : baseYearlyPrice,
    proratedDiscount: options.subscription === 'monthly' ? monthlyProratedDiscount : yearlyProratedDiscount,
    total,
  };
};

export const objectToFormData = (obj, key, formData) => {
  let fd = formData || new FormData();
  Object.keys(obj).forEach((k) => {
    if (Array.isArray(obj[k])) {
      obj[k].forEach((item, idx) =>
        fd.append(key ? `${key}.${k}[${idx}]` : `${k}[${idx}]`, item),
      );
    } else if (typeof obj[k] === 'object' && !(obj[k] instanceof File)) {
      fd = objectToFormData(obj[k], k, fd);
    } else if (obj[k]) fd.append(key ? `${key}.${k}` : k, obj[k]);
  });
  return fd;
};

export const removeHash = (filename) => {
  const newName = filename.split('---').reverse()[0];
  return newName;
};
